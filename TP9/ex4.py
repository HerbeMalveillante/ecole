class Livre():
    ...
    def prixParPage(self):
        print(f"Le prix de '{self.titre}' est de {round(self.prix / self.nbPages, 3)}€ par page.")
    ...
    def revientPlusCher(self, autreLivre):
        ...
        if prixLivre1 == prixLivre2:
            ...
            print(f"Le prix par page de {self.titre if prixLivre1 > prixLivre2 else autreLivre.titre} est plus cher que {self.titre if prixLivre1 < prixLivre2 else autreLivre.titre}")
    ...
    def auteursCommuns(self, autreLivre):
        if self.aCommun(self.liste_auteur, autreLivre.liste_auteur):
            print(f"Les auteurs en commun sont : {self.extraction(set(self.liste_auteur) & set(autreLivre.liste_auteur))}")
        else:
    ...
    def afficheAuteursEditeurs(cls, zone, editor_number):
        ...
        print(f"Les auteurs de livres édités par l'éditeur {editor_number} dans la zone {zone} sont : {cls.extraction(set(authorList))}")



l1 = Livre("978-2-1234-5678-9", "Le Petit Prince", ["Antoine de Saint-Exupéry"], 336, 24.99)
l2 = Livre("979-3-4321-8765-0", "Voyage au centre de la terre", ["Jules Verne", "Mon Daron"], 320, 19.99)
l3 = Livre("124-3-4321-0987-4", "Histoire des locomotives", ["Mon Daron", "Jean-Paul Sartre"], 276, 2)
