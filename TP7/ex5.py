...
def checkStock():
    ...
    print(f"Il y a {inventaire[fruit]} {fruit}s." if fruit in inventaire else "Desolé, le fruit n'est pas en stock")

checkStock()

def ajouteStock():
    fruit, stock = input("Entrez un fruit et son stock séparés par un espace : ").split()
    stock = int(stock)

    if fruit in inventaire and stock != inventaire[fruit]:
        print("Erreur : le fruit est déjà présent dans l'inventaire avec un stock différent")
        return
