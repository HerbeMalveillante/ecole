...
def exercice4():
    ...
    def crible(maxi):
        return [i if estPremier(i) else 0 for i in range(1, maxi)]

    def crible_list(maxi):
        return [j for j in [i if estPremier(i) else None for i in range(2, maxi)] if j]

    def verifList(list):
        return all([estPremier(i) for i in list])
...
def exercice7():
    def verification_base(chaine, base):
        return all([int(i) < base for i in chaine])
...
def exercice10():
    def triangle(etages):
        ...
        while spaces >= 0:
            output.append(
                " " * spaces + ((2 * etages - 1) - 2 * spaces) * "X" + " " * spaces
            )
            spaces -= 1
