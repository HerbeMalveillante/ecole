def affiche(listeChaines1, listeChaines2):
    if len(listeChaines1) != len(listeChaines2): # si les deux listes n'ont pas la même longueur
        print("Pb : nombre d'éléments différents") # on ne fait rien et on affiche une erreur
    else:
        print("Scientifiques :")
        for i in range(len(listeChaines1)): # pour chaque index dans la liste
            s = listeChaines1[i]+" "+listeChaines2[i] # on créée un nom de scientifique avec le nom de chaque liste en position i
