...
def saisie_lat():
    # Degrés
    entreeDeg = int(input("Pour la Latitude, entrer les degrés (42 <= entier <= 51 ) : "))
    while not 42 <= entreeDeg <= 51:
        entreeDeg = int(input("Incorrect, recommencez : "))
    
    # Minutes
    entreeMin = int(input("Pour la Latitude, entrer les minutes (0 <= entier <= 59 ) : "))
    while not 0 <= entreeMin <= 59:
        entreeMin = int(input("Incorrect, recommencez : "))
    
    # Secondes
    entreeSec = int(input("Pour la Latitude, entrer les secondes (0 <= entier <= 59 ) : "))
...
def saisie_long():
    ...
    while orientation not in ["E", "W"]:
        orientation = input("Réessayer : l'orientation choisie est incorrecte. (E/W) : ")
    ...
    # Minutes
    entreeMin = int(input("Pour la Longitude, entrer les minutes (0 <= entier <= 59 ) : "))
    while not 0 <= entreeMin <= 59:
        entreeMin = int(input("Incorrect, recommencez : "))
    
    # Secondes
    entreeSec = int(input("Pour la Longitude, entrer les secondes (0 <= entier <= 59 ) : "))
    while not 0 <= entreeSec <= 59:
        entreeSec = int(input("Incorrect, recommencez : "))
...
def distance(latv1, latv2, longv1, longv2):
    return 6378167 * math.acos(math.sin(latv1) * math.sin(latv2) + math.cos(latv1) * math.cos(latv2) * math.cos(longv1 - longv2))
